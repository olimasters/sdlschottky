...
{	//TODO: Change this into two classes by making the group theory more self-contained
	public:
		...
		Schottky(double a,double b,int pixWidth,int pixHeight);	//Constructor
...
Schottky::Schottky(double a,double b,int pixWidth,int pixHeight)
{
	this->a=a;
	...
	setb(b);	//Makes sure that updateParams() is called
}

void Schottky::seta(double a)
{
	this->a=a;
	updateParams();
}

void Schottky::setb(double b)
{
	this->b=b;
	updateParams();
}
...
{
	//We hav the radii, namely 1/(yv) and 1/y
	//We write a = 1/(yv), b=1/y
...
int main(int argc,char *argv[])
{
	...
	Schottky group(2.0,2.0,pixWidth,pixHeight);
	group.setWidth(8.0);
	group.setHeight(8.0);
	...
	for(bool quitting = false; !quitting;)
	{
		...
		switch(event.type)
		{
			...
			case SDL_MOUSEMOTION:
				...
				group.seta(params.imag());
				group.setb(params.real());
				group.plot();
				break;
		}
	}
